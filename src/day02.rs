...
fn choose_strategy((their_move, desired_result): (Move, Result)) -> Round {
...
pub fn run() {
    ...
    let part_2_score: i32 = read_lines().map(parse_move_and_result).map(choose_strategy).map(score).sum();
